...
annual_income = st.number_input("Annual Income", min_value=0, max_value=200000, value=50000)  # ✅ allow below 10000
...
if st.button("Predict Loan Eligibility"):
    # ✅ Rule: Reject if income < 10000
    if annual_income < 10000:
        st.error("❌ Loan Status Prediction: Rejected")  # red color output
    else:
        # Encode inputs
        ...
        if result == "Rejected":
            st.error(f"❌ Loan Status Prediction: {result}")  # red color
        else:
            st.success(f"✅ Loan Status Prediction: {result}")  # green color
